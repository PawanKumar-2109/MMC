...
import "../node_modules/chainlink/v0.5/contracts/vendor/Ownable.sol";
...
contract ParametricIns is ChainlinkClient, Ownable {
  ...
  string constant WORLD_WEATHER_ONLINE_KEY = "d22d793eadf640a2bff92644212806";
  ...
  constructor(address _mmctoken_contract_address,address payable _client, string memory _policy_number,  uint _duration, uint _premium, uint _payoutValue, string memory _cropLocation, string memory _lat, string memory _lon, uint _rainThreshold)  payable Ownable() public {
    ...
    emit contractCreated(insurer, _client, _policy_number, _duration, _premium, _payoutValue);
    
  }
    event contractCreated(address _insurer, address _client, string _policy_number, uint _duration, uint _premium, uint _totalCover);
    event dataRequestSent(bytes32 requestId);
    event dataReceived(uint _rainfall);
    event contractPaidOut(address _contract, address client, string _policy_number, uint _paidTime, uint _totalPaid, uint _finalRainfall);
  ...
  /**
    ...
    function checkRainfallCallBack(bytes32 _requestId, uint256 _rainfall) public recordChainlinkFulfillment(_requestId) {
       ...
       if (dataRequestsSent > 0) {
          ...
          if (currentRainfall < rainThreshold ) { //temp threshold has been  met, add a day of over threshold
              daysWithoutRain += 1;
          } else {
    ...
    function payOutContract() private   {
        ...
        emit contractPaidOut(address(this), client, policy_number, now, payoutValue, currentRainfall);
